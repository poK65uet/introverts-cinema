...
export default function UserProfile() {
  ...
  } = useChangeProfile(
    {
      ...
      phone:
        userData?.phone != store.login.user?.phone
          ...
          : undefined,
      birthDay:
        userData?.birthDay != store.login.user?.birthDay
          ? userData?.birthDay
          : undefined,
    },
  ...
  const handleSaveProfile = () => {
    if (
      !isValidPhoneString(userData?.phone ? userData.phone : '') &&
      userData?.phone
    ) {
      notify({
        type: 'error',
  ...
  const validate = (fieldValues = values) => {
    ...
    if ('password' in fieldValues) {
      ...
      if (fieldValues.password.length == 0) {
        tmp.password = 'Vui lòng điền mật khẩu của bạn';
      }
    }
    ...
    if ('newPassword' in fieldValues) {
      ...
      if (fieldValues.newPassword.length == 0) {
        tmp.newPassword = 'Vui lòng điền mật khẩu mới của bạn';
      } else {
        if (fieldValues.newPassword.length < 8) {
          tmp.newPassword = 'Mật khẩu phải có ít nhất 8 ký tự';
        }
      }
    }
    ...
    if ('reNewPassword' in fieldValues) {
      tmp.reNewPassword = '';
      if (fieldValues.reNewPassword.length == 0) {
        tmp.reNewPassword = 'Vui lòng nhập lại mật khẩu của bạn';
      } else {
        if (fieldValues.reNewPassword !== values.newPassword) {
          tmp.reNewPassword = 'Mật khẩu nhập lại không khớp';
        }
      }
    }
  ...
  } = useVerifyPassword(values.password, { onError: onVerifyPasswordError });
  ...
  } = useChangePassword(values.newPassword, { onError: onChangePasswordError });
              ...
              <Grid xs={12} container rowSpacing={1}>
                <Grid xs={5}>
                  <CustomInput.TextField
                    ...
                    onChange={(event: any) => {
                      handleInputChange(event), resetVerifyPassword();
                    }}
                ...
                <Grid xs={7} alignSelf="end" textAlign="end">
                  <Button
                    ...
                    disableRipple
                    className={classes.saveButton}
                    disabled={isPasswordVerfied != true}
